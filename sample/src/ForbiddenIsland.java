...
class Cell {
    ...
    Color getColor(int waterheight) {
        ...
        if (index > 0) {
            ...
            if (this.top.isFlooded || this.bottom.isFlooded || this.left.isFlooded || this.right.isFlooded) {
                this.isFlooded = true;
                if ((255 + 7 * index) < 0) {
    ...
    WorldImage drawCell(int waterHeight) {
        return new RectangleImage(CELL_SIZE, CELL_SIZE, OutlineMode.SOLID, this.getColor(waterHeight));
    }
}
...
class ForbiddenIslandWorld extends World {
    ...
    ForbiddenIslandWorld() {
        ...
        this.targets = new Cons<Target>(new Helicopter(ISLAND_SIZE / 2 + 1, ISLAND_SIZE / 2 + 1), util.renderTargets(mountain));
    ...
    public WorldScene drawCells() {
        ...
        for (Cell c : this.board) {
            w.placeImageXY(c.drawCell(this.waterHeight), Cell.CELL_SIZE * c.x - 5, Cell.CELL_SIZE * c.y - 5);
        }
    ...
    public WorldScene drawTargets(WorldScene given) {
        for (Target t : this.targets) {
            given.placeImageXY(t.drawTarget(), Cell.CELL_SIZE * t.x - 5, Cell.CELL_SIZE * t.y - 5);
        }
        return given;
    ...
    public WorldScene makeScene() {
        ...
        addTargets.placeImageXY(this.player.drawPlayer(), Cell.CELL_SIZE * this.player.x - 5, Cell.CELL_SIZE * this.player.y - 5);
        return addTargets;
    ...
    public void onKeyEvent(String ke) {
        if (ke.equals("m")) {
            ...
            this.waterHeight = 0;
            this.Tickcount = 0;
            ArrayList<ArrayList<Cell>> randommountain = this.transformArraytoCell(this.makeRandomMountain());
            util.getNeghbours(randommountain);
            this.targets = new Cons<Target>(new Helicopter(ISLAND_SIZE / 2 + 1, ISLAND_SIZE / 2 + 1), util.renderTargets(randommountain));
            ...
            this.targets = new Cons<Target>(new Helicopter(ISLAND_SIZE / 2 + 1, ISLAND_SIZE / 2 + 1), util.renderTargets(randomterrain));
            ...
            else if (this.board.find(f, x - 1, y).height - this.waterHeight <= 0) {
                return;
            }
            ...
            else if (this.board.find(f, x, y - 1).height - this.waterHeight <= 0) {
                return;
            }
