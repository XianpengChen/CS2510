...
class ArrayUtils {
    <T> ArrayList<T> insert(ArrayList<T> arr, T val, IComparator<T> comp) {
       
        int temp = 0;
        while (temp < arr.size()) {
            T val1 = arr.get(temp);
            if (comp.compare(val1, val) < 0) {
                temp++;
            }
            else {
                arr.add(temp, val);
    ...
    int partion(ArrayList<T> arr, IComparator) {
        
    }
   //lowIndex = 0, highIndex = arr.size() - 1;
    <T> ArrayList<T> mergeSort(ArrayList<T> arr, int lowIndex, int highIndex, IComparator<T> comp) {
        if (lowIndex >= highIndex) {
            ...
            ArrayList<T> arr1 = this.getRange(arr, 0, midIndex);
            ArrayList<T> arr2 = this.getRange(arr, midIndex + 1, highIndex);
            ArrayList<T> arr3 = this.mergeSort(arr1, 0, midIndex, comp);
            ArrayList<T> arr4 = this.mergeSort(arr2, 0, highIndex - midIndex - 1, comp);
...
class ExampleArrayLists {
    ...
    IComparator<Integer> intcom = new IntComparator();
    ArrayList<Integer> arr2 = u.mergeSort(an, 0, 10, intcom);
    ArrayList<Integer> arr3 = u.insert(arr2, 8, intcom);
